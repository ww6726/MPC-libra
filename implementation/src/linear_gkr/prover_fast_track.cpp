	...
	void prover::get_matrices( prime_field::field_element** A_from_verifier, prime_field::field_element** B_from_verifier, int matrix_size_from_verifier)
	{
		...
		for (int i = 0; i < matrix_size; ++i)
		{
			...
			{
				matrix_rand[i][j] = prime_field::random();
	...
	prime_field::field_element prover::merge_setup()
	{
		...
		inet_pton(AF_INET, "172.18.0.2", &serverAddr.sin_addr);
		...
		for (int attempt = 1; attempt <= maxRetries; ++attempt) {
			...
			if (attempt == maxRetries) {
				std::cerr << "Error connecting to server after " << maxRetries << " attempts\n";
				close(clientSocket);
				return EXIT_FAILURE;
			}

			// Sleep for a short duration before retrying
			std::cout << "Retrying connection (attempt " << attempt << ")...\n";
		...
		// // while (true) {
		//     if (send(clientSocket, &sentData, sizeof(sentData), 0) == -1) {
		//         std::cerr << "Error sending data\n";
		//     }

		//     std::cout << "Sent: " << sentData.to_gmp_class() << std::endl;

		//     if (recv(clientSocket, &receivedData, sizeof(receivedData), 0) == -1) {
		//         std::cerr << "Error receiving data\n";
		//     }

		//     std::cout << "Received: " << receivedData.to_gmp_class() << std::endl;
		// // }
	...
	prime_field::field_element* prover::matmul(prime_field::field_element** A, prime_field::field_element** B){
		...
		// print_vector(A_merge_flat,matrix_size*matrix_size);
		// print_vector(B_merge_flat,matrix_size*matrix_size);
	...
	quadratic_poly prover::sumcheck_phase1_update(prime_field::field_element previous_random, int current_bit)
	{
		...
		//merge the result
		ret.a =  merge_proof(ret.a);
		ret.b =  merge_proof(ret.b);
		ret.c =  merge_proof(ret.c);
